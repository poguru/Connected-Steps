...
export default function CheckinLoginPage() {
  ...
  async function login(e: React.FormEvent) {
    ...
    try {
      ...
      if (d.role === "checkin_team" || d.role === "event_admin") {
        router.push("/it-run/checkin");
      } else {
        setError("You do not have access to the check-in portal.");
      }
    } catch {
    ...
    <div style={{ minHeight: "100vh", background: "#080808", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, fontFamily: "'Inter',system-ui,sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 380 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 13, color: "#10b981", fontWeight: 600, marginTop: 4 }}>Event Day Check-in Portal</div>
        </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, padding: 28 }}>
          <form onSubmit={login}>
            ...
            <button type="submit" disabled={loading}
              style={{ width: "100%", padding: "12px", background: loading ? "rgba(255,255,255,0.1)" : "#10b981", border: "none", borderRadius: 8, color: "#fff", fontSize: 14, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
        ...
        <div style={{ textAlign: "center", marginTop: 20, fontSize: 12, color: "#555" }}>
          Check-in Team &amp; Event Admin only
        </div>
      </div>
    </div>
  );
