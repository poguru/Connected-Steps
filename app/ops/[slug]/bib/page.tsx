...
export default function BibPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    fetch("/api/ops/auth")
      ...
      .then((s: Session | null) => {
        ...
        fetch(`/api/ops/events/${s.eid}/live-stats`)
          .then(r => r.ok ? r.json() : null)
          .then((d: { stats?: { bib?: { done: number } } } | null) => {
            if (d?.stats?.bib) setCount(d.stats.bib.done);
          })
          .catch(() => {});
      })
  ...
  return (
    <div style={pageStyle}>
      <OpsHeader slug={slug} eventTitle={eventTitle} role={session.role} volunteerName={session.name}
        countLabel={count > 0 ? `${count} collected` : undefined} accentColor={ACCENT} />
      <ScannerModule eventId={session.eid} service="bib" serviceLabel="BIBs Collected" accentColor={ACCENT} onScanCount={setCount} />
    </div>
  );
}
...
function Spinner() {
  return (
    <div style={{ minHeight: "100vh", background: "#080808", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: 32, height: 32, borderRadius: "50%", border: "3px solid #222", borderTopColor: "#60a5fa", animation: "spin .7s linear infinite" }} />
