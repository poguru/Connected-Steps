...
export default function CertificatePage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    fetch("/api/ops/auth")
      ...
      .then((s: Session | null) => {
        ...
        fetch(`/api/ops/events/${s.eid}/live-stats`)
          .then(r => r.ok ? r.json() : null)
          .then((d: { stats?: { certificate?: { done: number } } } | null) => {
            if (d?.stats?.certificate) setCount(d.stats.certificate.done);
          })
          .catch(() => {});
      })
  ...
  return (
    <div style={pageStyle}>
      ...
      <ScannerModule eventId={session.eid} service="certificate" serviceLabel="Certificates Issued" accentColor={ACCENT} onScanCount={setCount} />
    </div>
  );
}
