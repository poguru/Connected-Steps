...
export default function BreakfastPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    fetch("/api/ops/auth")
      ...
      .then((s: Session | null) => {
        ...
        fetch(`/api/ops/events/${s.eid}/live-stats`)
          .then(r => r.ok ? r.json() : null)
          .then((d: { stats?: { breakfast?: { done: number } } } | null) => {
            if (d?.stats?.breakfast) setCount(d.stats.breakfast.done);
          })
          .catch(() => {});
      })
  ...
  return (
    <div style={pageStyle}>
      ...
      <ScannerModule eventId={session.eid} service="breakfast" serviceLabel="Breakfasts Issued" accentColor={ACCENT} onScanCount={setCount} />
    </div>
  );
}
...
function Spinner() {
  return (
    <div style={{ minHeight: "100vh", background: "#080808", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: 32, height: 32, borderRadius: "50%", border: "3px solid #222", borderTopColor: "#f97316", animation: "spin .7s linear infinite" }} />
