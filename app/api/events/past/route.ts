...
// GET /api/events/past â€” recently completed published events (last 30 days)
export async function GET() {
  ...
  const { data, error } = await db
    .from("events")
    .select("id, title, description, event_type, cover_image, start_date, start_time, end_date, end_time, registration_closes_at, location, organizer, price, share_slug, status, distance_categories, max_participants, participant_count")
    .eq("status", "published")
    .or(
      `end_date.lt.${today},` +
