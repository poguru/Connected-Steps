...
import { Card, Button, Label, Alert, Badge, EmptyState, Spinner } from "@/components/ui/ds";
...
const S: React.CSSProperties = { padding: "10px 12px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#fff", fontSize: 13, outline: "none", fontFamily: "inherit", width: "100%", boxSizing: "border-box" as const };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff", fontFamily: "inherit" }}>
      <header style={{ position: "sticky", top: 0, zIndex: 40, background: "rgba(10,10,10,0.97)", borderBottom: "1px solid rgba(255,255,255,0.07)", padding: "0 2rem", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <Link href={`/admin/events/${eventId}/manage`}><Button size="sm" variant="ghost">Back to Hub</Button></Link>
      </header>

      <div style={{ maxWidth: 860, margin: "0 auto", padding: "1.75rem 2rem" }}>

        {/* ── Add / Edit form ── */}
        <Card style={{ marginBottom: 24, borderColor: "rgba(232,98,10,0.2)" }}>
          <Badge color="orange" style={{ marginBottom: 16 }}>{editing ? "Edit Sponsor" : "Add Sponsor"}</Badge>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div><Label>Name *</Label><input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Decathlon India" style={S} /></div>
            <div><Label>Tier</Label><select value={form.tier} onChange={e => setForm(f => ({ ...f, tier: e.target.value }))} style={{ ...S, cursor: "pointer" } as React.CSSProperties}>{TIERS.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}</select></div>
            <div><Label>Logo URL</Label><input value={form.logo_url} onChange={e => setForm(f => ({ ...f, logo_url: e.target.value }))} placeholder="https://…/logo.png" style={S} /></div>
            <div><Label>Website URL</Label><input value={form.website_url} onChange={e => setForm(f => ({ ...f, website_url: e.target.value }))} placeholder="https://sponsor.com" style={S} /></div>
            <div style={{ gridColumn: "1/-1" }}><Label>Description</Label><input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Short description for event page" style={S} /></div>
            <div><Label>Display Order</Label><input type="number" value={form.display_order} onChange={e => setForm(f => ({ ...f, display_order: Number(e.target.value) }))} style={S} /></div>
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <Button loading={saving} onClick={save}>{editing ? "Save Changes" : "Add Sponsor"}</Button>
        ...
        {loading ? (
          ...
          <EmptyState icon="🤝" title="No sponsors added yet" body="Sponsors will appear on the event page and in emails" />
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {sponsors.map(s => (
              <Card key={s.id} style={{ display: "flex", alignItems: "center", gap: 14, opacity: s.visible ? 1 : 0.5 }}>
                {s.logo_url ? (
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 4, flexWrap: "wrap" as const }}>
                    <Badge size="sm" style={{ color: tierColor(s.tier), background: `${tierColor(s.tier)}18`, borderColor: `${tierColor(s.tier)}30` }}>{tierLabel(s.tier)}</Badge>
                ...
                <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                  <Button size="xs" variant={s.visible ? "ghost" : "secondary"} onClick={() => toggleVisible(s)}>{s.visible ? "Visible" : "Hidden"}</Button>
                  <Button size="xs" variant="ghost" onClick={() => startEdit(s)}>Edit</Button>
                  <Button size="xs" variant="danger" onClick={() => deleteSponsor(s.id)}>Remove</Button>
                </div>
              </Card>
            ))}
          </div>
